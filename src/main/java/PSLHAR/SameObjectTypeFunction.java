...
class SameObjectTypeFunction implements ExternalFunction {

	private ArgumentType[] argTypes = new ArgumentType[]{ArgumentType.UniqueID, ArgumentType.UniqueID, ArgumentType.UniqueID, ArgumentType.UniqueID};
	private static final ArrayList<Integer> values = new ArrayList<Integer>();

	public SameObjectTypeFunction() {	}
	...
	public double getValue(ReadOnlyDatabase db, GroundTerm... args) {
		...
		try{
			...
			if(values.size()<2){
				System.err.println("Error in SameObjectTypeFunction: missing extra input variable ");
				System.exit(-1);
			}
			else{
				int i =1;			
            ...
            System.err.println("Error in SameObjectTypeFunction: " + e.getMessage());
        }
